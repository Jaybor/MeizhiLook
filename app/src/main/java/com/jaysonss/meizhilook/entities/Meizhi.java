...
public class Meizhi implements Parcelable {

    public static final Creator<Meizhi> CREATOR = new Creator<Meizhi>() {
        @Override
        public Meizhi createFromParcel(Parcel source) {
            return new Meizhi(source);
        }

        @Override
        public Meizhi[] newArray(int size) {
            return new Meizhi[size];
        }
    };
    /**
     * error : false
    ...
    protected Meizhi(Parcel in) {
        this.error = in.readByte() != 0;
        this.results = in.createTypedArrayList(DataInfo.CREATOR);
    }
